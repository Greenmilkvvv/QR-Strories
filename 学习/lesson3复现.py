...
# 第1节 在 __init__() 中提前计算指标

'''
Strategy 中的 __init__() 函数在回测过程中只会在最开始的时候调用一次, 
而 next() 会每个交易日依次循环调用多次。

所以为了提高回测效率: 
建议先在 __init__() 中一次性计算好指标（甚至是交易信号）, 
然后在 next() 中调用已经算好的指标, 
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self): 
        ...
        self.close_over_ema = self.notify_data.close > self.ema1
...
'''
调用指标时会设计 line 的索引和切片操作, 可能有点麻烦. 
简便起见, 在 next() 中调用当前时刻指标时, 可以省略索引 [0] : 

即在 next() 中, 
- self.sma5[0] 等价于 self.sma5 
- self.data.close[0] 等价于 self.data.close
...
class TestStrategy(bt.Strategy):
    def __init__(self): 
        ...
        # datas[0] 表示第一个证券
        # lines[0] 表示第一个证券的第一个指标, 即 close 线
...
ed_date = datetime.datetime(2021, 10, 12)
...
Backtrader 提供了丰富的运算函数, 方便用户在编写策略时使用: 
And、Or、If、All、Any、Max、Min、Sum、Cmp...
...
class TestStrategy(bt.Strategy): 
    def __init__(self): 
        ...
        # 5, 10日均线
        self.sma5 = btind.MovingAverageSimple(period=5)
        self.sma10 = btind.MovingAverageSimple(period=10)
...
# 第4节 如何对齐不同周期的指标
"""
通常情况下, 操作的都是相同周期的数据, 比如: 
日度行情数据计算返回各类日度指标、周度行情数据计算返回各类周度指标
...
class TestStrategy(bt.Strategy): 
    def __init__(self):
        ...
        # Xxx 对应源码里已注册好的指标类（SMA、EMA、RSI、MACD...）
...
'''
为了满足用户使用习惯, Backtrader 接入了 TA-Lib 技术指标库, 

官方 document : https://www.backtrader.com/docu/talibindautoref/ , 其对于

- 各个函数的输入、输出
- 在 Backtrader 中特有的绘图参数
- 返回的 lines 属性等信息
...
# 第6节 自定义新指标
'''
在 Backtrader 中, 涉及到自定义操作, 一般都是通过
- 继承原始的父class, 然后
- 在新的子class里自定义属性, 
...
class MyInd(bt.Indicator): 
    ...
    # 定义指标函数返回的 lines 名称, 方便后面通过名称调用具体的指标, 
    # 如 self.lines.xxx、self.l.xxx、self.xxx
    lines = ('xxx', 'zzz')

    # 定义参数 params, 方便在子类里全局调用, 也方便在使用指标函数时修改参数取值；
    ## 声明可调参数及其默认值, 类似于函数的默认参数。
    n=5 # 默认为5
    params = (('xxx', n),) # 最后一个 “,” 别省略

    # __init__() 方法：
    # 同策略 Strategy 里的 __init__() 类似, 
    # 对整条 line 进行运算, 运算结果也以整条 line 的形式返回；
    def __init__(): 
        '''可选'''
        pass

    # next() 方法：
    # 同策略 Strategy 里的 next() 类似, 
    # 每个 bar 都会运行一次, 在 next() 中是对数据点进行运算；
    def next(self):
        '''可选'''
        pass

    # once() 方法：
    # 这个方法只运行一次, 但是需要从头到尾循环计算指标；
    def once(self):
        '''可选'''
        pass 

    # 指标绘图相关属性的设置：例如：
    # plotinfo = dict() 通过字典形式修改绘图参数；
    # plotlines = dict() 设置曲线样式等...
    # plotinfo = dict(...)
    # plotlines = dict(...)
...
"""
重要提示：自定义指标时, 建议首选 __init__(), 

因为 __init__() 最智能, 
能自动实现 next() 和 once() 的功能, 
计算指标一气呵成
"""
...
class DummyInd(bt.Indicator): 
    ...
    lines = ('dummyline', ) # 给 lines 之下注册一个新 line 名叫 dummyline

    # 定义参数, 后面调用这个参数的方式有：
    ## self.params.xxx 、 
    ## self.p.xxx

    params = (('value', 5), ) # 申明一个参数 默认值为5

    def __init__(self): 
        # __init__() 中是对 line 进行运算，
        # 最终也以 line 的形式返回，
        # 所以运算结果直接赋值给了 self.l.dummyline；
        self.lines.dummyline = bt.Max(0.0, self.p.value)

    def next(self): 
        # next() 中是对当前时刻的数据点进行运算（用了常规的 max() 函数），
        # 返回的运算结果也只是当前时刻的值，

        # 所以是将结果赋值给 dummyline 的当前时刻：self.l.dummyline[0]， 
        # 然后依次在每个 bar 都运算一次；

        self.lines.dummyline[0] = max(0.0, self.p.value)
